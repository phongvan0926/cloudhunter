...
const SEA = ['STATIC', 'FLOWING', 'FLUCTUATING', 'ROLLING', 'FOG'];

rankSpotsForDawn(DATE, m => console.log('   ' + m)).then(rows => {
  console.log(`\n🌄 Xếp hạng rạng sáng ${DATE} — ${rows.length} điểm\n`);
  rows.slice(0, 15).forEach((r, i) => {
    console.log(`${String(i + 1).padStart(2)}. ${String(r.score).padStart(3)}/100  ${r.status.padEnd(12)} `
      + `${r.name.slice(0, 34).padEnd(36)} đứng ${String(r.elevation).padStart(4)}m / đáy ${String(r.valleyElev).padStart(4)}m  đồng thuận ${r.agreement}%`);
  });
  const sea = rows.filter(r => SEA.includes(r.status));
  const worth = rows.filter(r => r.score >= WORTH_GOING_SCORE);
  console.log(`\n   ${sea.length}/${rows.length} điểm có trạng thái thuộc nhóm CÓ MÂY, `
    + `${worth.length} điểm đạt ngưỡng "đáng đi" (${WORTH_GOING_SCORE}/100)`);
